...
const BASEURL_MIRRORS = [
    'https://otakudesu.best',
    'https://otakudesu.site',
];
...
const scrapeEpisode = async (html) => {
    ...
    try {
        ...
        console.log(`[Stream] Searching External: ${cleanTitle} | Eps: ${episodeNumber}`);
        streamResolutions = await getExternalStream(cleanTitle, episodeNumber);
    } catch (err) {
        console.log("[Stream] API Eksternal Error/Skip.");
    }

    if (!streamResolutions || Object.keys(streamResolutions).length === 0) {
        console.log(`[Stream] API Eksternal nihil. Mencoba scrape lokal...`);
        try {
            streamResolutions = await getLocalStreamQuality($);
        } catch (e) {
            console.error("[Stream] Scrape lokal gagal:", e.message);
        }
    }
...
const ongoingAnimeFunc = async (page = 1) => {
    return await retryWithBackoff(async () => {
        ...
        console.log(`[ONGOING] Fetching: ${url}`);
...
const genreListsFunc = async () => {
    return await retryWithBackoff(async () => {
        const { data } = await axiosInstance.get(`${BASEURL}/genre-list`, {
            timeout: 12000
        });
...
const episodeFunc = async ({ episodeSlug, animeSlug, episodeNumber }) => {
    let slug = '';
    console.log(`[API] Request: Anime=${animeSlug}, Ep=${episodeNumber}, Slug=${episodeSlug}`);
    ...
    if (animeSlug) {
        ...
        if (cachedData && (now - cachedData.timestamp < CACHE_DURATION)) {
            console.log(`[CACHE] HIT: ${animeSlug}`);
            episodeLists = cachedData.list;
        } else {
            console.log(`[CACHE] MISS: Scraping halaman anime ${animeSlug}...`);
        ...
        console.log(`[RESULT] Final slug resolved: ${slug}`);
    }

    try {
        console.time("FetchEpisodePage");
        const { data } = await retryWithBackoff(async () => {
            return await axiosInstance.get(`${BASEURL}/episode/${slug}`, {
                timeout: 10000
            });
        });
        console.timeEnd("FetchEpisodePage");

        return await scrapeEpisode(data);

    } catch (error) {
        console.error(`[ERROR] Gagal fetch episode ${slug}:`, error.message);
        return undefined;
    }
};
...
const moviesFunc = async (page = 1) => {
    return await retryWithBackoff(async () => {
        ...
        console.log(`[MOVIES] Fetching: ${movieUrl}`);
...
const movieFunc = async (slug) => {
    return await retryWithBackoff(async () => {
        console.log(`[MOVIE] Fetching: ${ANOBOY}${slug}`);
        ...
        $('.download .ud .udl').each((index, element) => {
            ...
            console.log(`[DOWNLOAD] ${label} → ${link}`);
...
// API v1 routes
app.get('/v1/', (_, res) => {
    res.status(200).json({ status: 'Ok', message: 'ANIMEKITA API BASE' });
});
...
// 404 handler
app.use((_, res) => {
    res.status(404).json({ status: 'Error', message: "Gak ada yang begituan bang >_<" });
});
